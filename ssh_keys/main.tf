...
resource "aws_key_pair" "mod" {
  ...
  public_key = "${file("${path.module}/portal_infra.pub")}"
}
...
output "private_key_path" { value = "${path.module}/portal_infra.pem" }
output "public_key_path"  { value = "${path.module}/portal_infra.pub" }
